...
class ImprovedHybridTradingEnv(gym.Env):
    ...
    def _load_data(self):
        ...
        for pair in self.pairs:
            df = pd.read_csv(
                os.path.join(self.data_dir, f"{pair}_hourly.csv"),
                index_col=0,
                parse_dates=True,
            )
    ...
    def _get_ml_signal(
        self, pair: str, current_step: int
        ...
        try:
            ...
            basic_features = [
                ...
                (
                    recent_data["SMA_10"].iloc[-1]
                    if "SMA_10" in recent_data.columns
                    else recent_data["4. close"].iloc[-1]
                ),
                (
                    recent_data["SMA_20"].iloc[-1]
            ...
            if self.ml_performance[pair]["total"] > 0:
                accuracy = (
                    self.ml_performance[pair]["correct"]
                    / self.ml_performance[pair]["total"]
                )
            else:
                accuracy = 0.5
    ...
    def _get_observation(self):
        ...
        if self.use_ml_signals:
            ...
            for pair in self.pairs:
                signal, confidence, accuracy, feedback = self._get_ml_signal(
                    pair, self.current_step
                )
