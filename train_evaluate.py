...
def prepare_dataset(df, pair, lookahead=2, threshold=0.005):
    ...
    df = df.dropna()
    features = [
        'SMA_10', 'SMA_20', 'EMA_10', 'EMA_20', 'RSI', 'Williams_%R', 'BB_High', 'BB_Low',
        'MACD', 'MACD_Signal', 'ATR', 'Stoch_K', 'Stoch_D', 'ADX', 'OBV', 'CCI', 'vix',
        '4. close_roll_mean5', '4. close_roll_std5', '4. close_roll_min5', '4. close_roll_max5',
        'RSI_roll_mean5', 'RSI_roll_std5', 'RSI_roll_min5', 'RSI_roll_max5',
        'MACD_roll_mean5', 'MACD_roll_std5', 'MACD_roll_min5', 'MACD_roll_max5',
        'ATR_roll_mean5', 'ATR_roll_std5', 'ATR_roll_min5', 'ATR_roll_max5',
        'vix_roll_mean5', 'vix_roll_std5', 'vix_roll_min5', 'vix_roll_max5',
        'SMA_10_lag1', 'SMA_10_lag2', 'RSI_lag1', 'RSI_lag2', 'MACD_lag1', 'MACD_lag2'
    ]
    ...
    y = np.where(pct_change > threshold, 1, np.where(pct_change < -threshold, 0, np.nan))
...
def train_and_evaluate(pair):
    ...
    if X is None or y is None or len(y) < 100:
        print(f"Failed to prepare dataset for {pair} or not enough samples after filtering.")
        return
    ...
    for train_idx, test_idx in tscv.split(X_scaled):
        ...
        print(f"Fold {fold}: Accuracy={metrics['accuracy'][-1]:.2f}, Precision={metrics['precision'][-1]:.2f}, Recall={metrics['recall'][-1]:.2f}, F1={metrics['f1'][-1]:.2f}")
        fold += 1
