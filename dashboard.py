...
st.markdown(
    '<div class="main-header">🤖 AI Trading Bot Dashboard v1.0.0</div>',
    unsafe_allow_html=True,
)
...
if dashboard_mode == "📈 Market Analysis":
    ...
    if performance_logs:
        ...
        fig.add_trace(
            go.Scatter(
                x=df_logs["timestamp"],
                y=df_logs["win_rate"].astype(float),
                mode="lines+markers",
                name="Win Rate",
        ...
        fig.add_trace(
            go.Scatter(
                x=df_logs["timestamp"],
                y=df_logs["avg_reward"].astype(float),
                mode="lines+markers",
                name="Avg Reward",
    ...
    assets = (
        list(historical_data.keys())
        if historical_data
        else ["AUDUSD", "GBPUSD", "USDJPY", "XAUUSD"]
    )
    ...
    config_info = {
        "Trading Pairs": (
            list(historical_data.keys()) if historical_data else "None loaded"
        ),
