...
def create_trading_bot_icon(size=256):
    ...
    for i, (pos, (high, low, open_price, close, bullish)) in enumerate(
        zip(candlestick_positions, candlestick_data)
    ):
    ...
    left_eye = patches.Rectangle(
        (4.0, 7.1), 0.4, 0.4, facecolor=secondary_color, edgecolor="none"
    )
    right_eye = patches.Rectangle(
        (5.6, 7.1), 0.4, 0.4, facecolor=secondary_color, edgecolor="none"
    )
    ...
    mouth_rect = patches.Rectangle(
        (4.3, 6.7), 1.4, 0.2, facecolor=secondary_color, edgecolor="none", alpha=0.8
    )
    ...
    # Control buttons
    button1 = Circle(
        (3.2, 5.5), 0.15, facecolor=accent_color, edgecolor="none", alpha=0.7
    )
    button2 = Circle((3.2, 5.0), 0.15, facecolor=red_color, edgecolor="none", alpha=0.7)
    button3 = Circle(
        (6.8, 5.5), 0.15, facecolor=secondary_color, edgecolor="none", alpha=0.7
    )
    button4 = Circle(
        (6.8, 5.0), 0.15, facecolor=primary_color, edgecolor="none", alpha=0.7
    )
...
def create_ico_file():
    ...
    # Save as .ico file
    images[0].save(
        "icon.ico", format="ICO", sizes=[(img.width, img.height) for img in images]
    )
...
def create_banner():
    ...
    # Right side - bot icon
    bot_circle = Circle(
        (10, 2), 0.8, facecolor=primary_color, alpha=0.3, edgecolor=primary_color
    )
